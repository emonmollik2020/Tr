...
# ডিফল্ট স্টেট (৫টি প্রো-লেভেল ফিল্টার অনুযায়ী সাজানো)
DEFAULT_STATE = {
    "price": 0.0,
    ...
    "sl_level": 0.0,
    "tp_level": 0.0,
    "analysis_15m": {"rsi": 0, "ema20": 0, "ema50": 0, "vwap": 0, "sig": "로드 হচ্ছে...", "pats": []},
    "analysis_1h": {"rsi": 0, "ema200": 0, "btc_price": 0, "sig": "로드 হচ্ছে...", "pats": []},
    "wait_reason": "로드 হচ্ছে...",
    "log": [],
    "history": []
}
...
# =====================================================================
# SECTION 4: সুইং ও ট্রেন্ড ট্রেডিং বট ইঞ্জিন (৫টি প্রফেশনাল লজিক সহ)
# =====================================================================
def bot_engine():
    ...
    while True:
        try:
            # ১৫ মিনিটের ক্যান্ডেলস্টিক ডাটা সংগ্রহ (লিমিট ১০০০, যাতে EMA 200 সঠিকভাবে কাজ করে)
            ...
            # ২. বিটগেট লাইভ অর্ডার বুক সংগ্রহ করা (Order Book Imbalance Filter)
            ...
            ob_imbalance = total_bids / max(0.001, total_asks)
            ob_confirmed = ob_imbalance > 1.15  # ক্রেতাদের চাপ অন্তত ১.১৫ গুণ বেশি হতে হবে
            ...
            btc_bullish = btc_p > btc_e20  # BTC যদি ১৫ মিনিটের EMA 20 এর উপরে থাকে
            ...
            atr_pct = atr / p
            dynamic_tp_pct = max(0.015, min(0.060, 2.5 * atr_pct))  # ২.৫ গুণ ATR, সর্বনিম্ন ১.৫%, সর্বোচ্চ ৬.০%
            dynamic_sl_pct = max(0.010, min(0.035, 1.5 * atr_pct))  # ১.৫ গুণ ATR, সর্বনিম্ন ১.০%, সর্বোচ্চ ৩.৫%
            ...
            # ক্রয়ের লজিক (BUY Condition - প্রো-লেভেল সুইং ট্রেডিং)
            ...
            # ৮টি কনফ্লুয়েন্স ফিল্টার (পদ্ধতিগত নিরাপত্তা)
            can_buy = (macro_bullish and 
                       ema_alignment and 
                       btc_bullish and 
            ...
            # বিক্রয়ের লজিক (SELL / Trend Reversal Exit)
            smart_sell = p < e50 or r15 > 78

            if in_pos:
                # ক. নো-লস সুরক্ষাকবচ (Breakeven Protection)
                # ডাইনামিক স্টপ লসের ৬০% লাভ হয়ে গেলেই বট স্টপ লসকে কেনার মূল্যে সেট করবে (ঝুঁকিমুক্ত সুইং ট্রেড)
                ...
                if p > peak_p:
                    peak_p = p
                    new_sl = round(p * (1 - initial_sl_dist_pct), 2)  # ডাইনামিক ট্রেইলিং স্টপ লস
                    if new_sl > cur["sl_level"]:
                        cur.update({"sl_level": new_sl})

                # গ. টেক প্রফিট (ডাইনামিক), স্টপ লস (ডাইনামিক) অথবা স্মার্ট সেল ট্রিগার হলে পজিশন ক্লোজ
                ...
                if can_buy:
                    ...
                    # ৭. ঝুঁকিভিত্তিক পজিশন সাইজিং (Kelly Criterion / Dynamic Position Sizing)
                    # মোট ব্যালেন্সের ২% স্টপ লসে হারাতে প্রস্তুত রেখে ডাইনামিক ট্রেড সাইজ (USD) হিসাব করা
                    account_balance = cur.get("balance", 100.0)
                    risk_amount = account_balance * 0.02
                    pos_size_usd = risk_amount / dynamic_sl_pct
                    pos_size_usd = max(10.0, min(account_balance, pos_size_usd)) # অ্যাকাউন্ট লিমিট বাউন্ডিং
                    ...
                    cur["log"].insert(0, {"t": datetime.now().strftime("%H:%M"), "m": f"🟢 BUY @ ${p:.2f} (Trade Size: ${pos_size_usd:.2f})"})
            
            # ড্যাশবোর্ডের জন্য স্টেট আপডেট (নতুন VWAP এবং BTC প্রাইস ডাটা সহ)
            cur.update({
                "price": round(p, 2),
            ...
            # ড্যাশবোর্ডের জন্য ওয়েটিং মেসেজ সাজানো (৫টি নতুন ফিল্টার সহ)
            if in_pos:
                ...
                cur["wait_reason"] = "অর্ডার বুক ইমব্যালেন্স (বিক্রেতাদের চাপ বেশি)"
